...
const ViTriForm = ({ location, onClose }) => {
    ...
    const handleImageUpload = async ({ file }) => {
        try {
            ...
            // Kiểm tra nếu location không tồn tại hoặc chưa có id
            if (!location || !location.id) {
                message.error("Vui lòng lưu vị trí trước khi tải ảnh lên!");
                return;
            }

            console.log("📤 Đang tải lên ảnh cho vị trí:", location.id);

            const response = await uploadLocationImage(location.id, file);
            setImage(response.content); // Cập nhật URL ảnh mới
