...
const ViTriForm = ({ location, onClose }) => {
    ...
    // Reset form khi mở form thêm mới
    useEffect(() => {
        if (!location) {
            form.resetFields(); // Xóa dữ liệu cũ trên form
            setImage(null); // Xóa ảnh preview
            setCurrentLocation(null); // Reset dữ liệu vị trí
        } else {
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            if (image) {
                ...
                updatedLocation.hinhAnh = location.hinhAnh; // Giữ nguyên ảnh cũ
            }
