...
const getHeaders = async () => {
    ...
    if (isTokenExpired(token)) {
        console.warn("Token hết hạn. Đang làm mới token...");
        token = await refreshToken(); // Gọi refreshToken để làm mới token
    }
...
export const addRoom = async (roomData) => {
    try {
        const headers = await getHeaders(); // Sử dụng getHeaders
        ...
        if (error.response?.status === 403) {
            ...
            return addRoom(roomData); // Thử lại với token mới
        }
